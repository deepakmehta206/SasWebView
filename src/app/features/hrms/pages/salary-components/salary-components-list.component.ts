...
@Component({
  ...
  templateUrl: './salary-components-list.component.html',
  styleUrl: './salary-components-list.component.scss'
})
export class SalaryComponentListComponent implements OnInit {
  private readonly api = inject(HrmsApiService);
  readonly permissionCodes = PermissionCodes;
  readonly loading = signal(true);
  readonly rows = signal<SalaryComponentDto[]>([]);
  ...
  load(): void {
    ...
    this.api.getSalaryComponents().subscribe({
      ...
      error: (error) => {
        this.rows.set([]);
        this.errorMessage.set(extractApiErrorMessage(error, 'Unable to load salary components.'));
        this.loading.set(false);
      }
    });
  }
