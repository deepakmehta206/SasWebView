...
export class BrandFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.getBrand(this.id).subscribe({
      next: (row) => {
      ...
      error: (error) => {
        this.errorMessage.set(extractApiErrorMessage(error, 'Unable to load brand.'));
        this.loading.set(false);
      }
    });
  }
  ...
  submit(): void {
    ...
    const createBody = {
      brandCode: value.brandCode.trim(),
      brandName: value.brandName.trim()
    };
    const updateBody = { ...createBody, isActive: value.isActive };
    const request$ = this.isEdit()
      ? this.api.updateBrand(this.id!, updateBody)
      : this.api.createBrand(createBody);
    request$.subscribe({
      next: () => {
      ...
      error: (error) => {
        this.errorMessage.set(extractApiErrorMessage(error, 'Unable to save brand.'));
        this.saving.set(false);
      }
    });
  }
